...
const TYPE_MAP: Record<string, { prisma: string; zod: string; ts: string }> = {
...
function parseFields(rawFields: string[]): FieldDef[] {
  return rawFields.map((raw) => {
    ...
    if (!TYPE_MAP[type]) {
      throw new Error(
        `Unknown type "${type}" in field "${raw}". Supported types: ${Object.keys(TYPE_MAP).join(", ")}`,
      );
    }
    return { name, type };
  });
...
export async function generateModel(modelName: string, rawFields: string[], cwd: string) {
...
async function writePrismaModel(
  className: string,
  tableName: string,
  fields: FieldDef[],
  cwd: string,
  ...
  if (!schema.includes(marker)) {
    throw new Error(
      `Could not find the "${marker}" marker in prisma/schema.prisma. Has it been edited?`,
    );
  }
...
async function writeModelFile(className: string, fields: FieldDef[], cwd: string) {
